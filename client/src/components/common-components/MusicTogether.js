...
function MusicTogether({ socket, partnerId, onExit }) {
  ...
  const handleVolumeChange = (e) => {
    const value = parseFloat(e.target.value); // 0.0 to 1.0
  ...
  // Apply volume to audio element (mobile-safe)
  useEffect(() => {
    if (audioRef.current) {
  ...
  // Ensure volume persists when a new song starts
  useEffect(() => {
    if (audioRef.current) {
      audioRef.current.volume = volume;
    }
  ...
  return (
    <div style={containerStyle}>
      ...
      <div style={volumeSidebarStyle} title="Volume">
