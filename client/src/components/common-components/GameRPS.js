...
const GameRPS = ({ isFirstPlayer, socket, partnerId, onGameEnd, updateScore, gameName = 'Rock Paper Scissors' }) => {
  ...
  useEffect(() => {
    if (!result || !socket || !partnerId) return;
    if (result !== 'win' && result !== 'lose') return;
    if (scoreUpdatedRef.current) return;
    scoreUpdatedRef.current = true;
    const winnerLabel = result === 'win' ? 'you' : 'partner';
    socket.emit('gameScore', { to: partnerId, winner: winnerLabel, game: 'rps' });
    if (typeof updateScore === 'function') {
      updateScore(winnerLabel, gameName || 'Rock Paper Scissors');
    }
  }, [result, socket, partnerId, updateScore, gameName]);
