  ...
  try {
    ...
    if (process.env.TURN_SECRET_KEY) {
      ...
      iceServers.push({
       ...
       console.warn('⚠️ TURN_SECRET_KEY not set. Serving STUN-only ICE configuration.');
    }
    ...
    const response = {
      ...
      metadata: {
        ...
        ttl: credentials ? credentials.ttl : null,
        expiresAt: credentials ? iceConfiguration.credentials.expiresAt : null,
        serverInfo: {
          turnServer: process.env.TURN_SECRET_KEY ? turnServerUrl : 'Not Configured',
          stunServer: stunServerUrl,
          realm: turnRealm
        },
        features: {
          tcpFallback: !!process.env.TURN_SECRET_KEY,
          tlsSupport: !!process.env.TURN_SECRET_KEY,
          iceRestart: true,
